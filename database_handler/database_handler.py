...
class DatabaseHandler(DBConnection):
    ...
    def get_tv_show_title(self, content_id) -> str:
        ...
        if query_result := self.get_data_from_db_first_result(query, (content_id,)):
            return query_result.get("title")

    def get_movie_title_list(self) -> dict:
        ...
        return {"media_list_content_type": ContentType.MEDIA.value,
                "media_list": self.get_data_from_db(query)}

    def get_tv_show_title_list(self) -> dict:
    ...
    def get_tv_show_metadata(self, content_id) -> dict:
        ...
        tv_show_metadata.update(self.get_data_from_db_first_result(tv_show_info_query, (content_id,)))
        tv_show_metadata.update(self.get_data_from_db_first_result(season_count_query, (content_id,)))
        tv_show_metadata.update(self.get_data_from_db_first_result(episode_count_query, (content_id,)))
    ...
    def get_tv_show_season_metadata(self, content_id) -> dict:
        ...
        tv_show_season_metadata.update(self.get_data_from_db_first_result(tv_show_info_query, (content_id,)))
        tv_show_season_metadata.update(self.get_data_from_db_first_result(episode_count_query, (content_id,)))
    ...
    def get_media_metadata(self, content_id) -> dict:
        ...
        if media_metadata := self.get_data_from_db_first_result(query, (content_id,)):
    ...
    def get_media_content(self, content_type, content_id=None):
        ...
        if content_type == ContentType.SEASON.value and content_id:
            ...
            media_metadata.update(self.get_movie_title_list())
        elif content_type == ContentType.TV.value:
            media_metadata.update(self.get_tv_show_title_list())
        else:
    ...
    def get_increment_episode_metadata(self, content_id, playlist_id, query_list):
        if list_index_query_result := self.get_data_from_db_first_result(query_list[0], (playlist_id, content_id,)):
            if next_media_query_result := self.get_data_from_db_first_result(query_list[1],
                                                                             (playlist_id, list_index_query_result.get(
                                                                                 "list_index"),)):
                return self.get_media_metadata(next_media_query_result.get("media_id"))
            else:
                if first_media_id_query_result := self.get_data_from_db_first_result(query_list[2], (playlist_id,)):
                    return self.get_media_metadata(first_media_id_query_result.get("media_id"))
        else:
    ...
    def get_media_folder_path(self, content_id):
        ...
        return self.get_data_from_db(query, (content_id,))

    def query_id(self, query: str, params: tuple):
        if query_result := self.get_data_from_db_first_result(query, params):
    ...
    def get_tv_show_id(self, content_id) -> int:
        ...
        return self.query_id(query, (content_id,))
    ...
    def get_playlist_media_id(self, playlist_id, content_id, list_index):
        ...
        return self.query_id(query, (playlist_id, content_id, list_index))
