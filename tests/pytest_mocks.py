...
test_config_file = {
    ...
    "media_folders": [
        ...
        {
            "content_src": "/media/library_3/",
            "content_url": "http://localhost:8003"
        }
    ]
}
...
def patch_get_free_disk_space_percent(test_class):
    patcher = patch('app.utils.common.get_free_disk_space_percent')
    test_class.get_free_disk_space_percent = patcher.start()
    test_class.get_free_disk_space_percent.return_value = 10
    test_class.addCleanup(patcher.stop)
...
def patch_load_json_file_content(test_class):
    patcher = patch('app.utils.config_file_handler.load_json_file_content')
    test_class.load_json_file_content = patcher.start()
    test_class.load_json_file_content.return_value = test_config_file
    test_class.addCleanup(patcher.stop)
